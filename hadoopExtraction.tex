...
% TODO grep map, reduce and italicize
% TODO bottom margin too big, make it smaller
% TODO for viva: reread hbase book doc on hfile
% TODO grep for model (in this chapter) and clarify if needed
% TODO grep for backoff back-off
% TODO grep for gram. use $n$-gram consistently
% TODO grep for filter filtering usage
% TODO grep for future and add it to future work in conclusion
% TODO check with Bill the 2000 words per minute ??????
% TODO check abbreviation space after dot (i.e. e.g. etc.)
% TODO grep for corpus (parallel corpus) and replace with parallel text
% TODO grep for MapReduce feature
% TODO grep for collection specific and add hyphen
% TODO subsubsections to be numbered
% TODO grep for word alignment and add or not hyphen
% TODO grep for Hfile wrongly capitalized
% TODO try to remove all mentions of hbase
% TODO check for duplicates in bibliography
% TODO introduce MapReduce in background chapter
% TODO check title casing
% TODO check usage of word ``model''
% TODO check usage of past tense and present tense
...
mentioned in \autoref{chap:intro}, translation even needs to be carried out in % TODO for online systems: maybe refer to intro (Google Translate, etc.)
...
and distributed computing models such as % TODO maybe refer to background
...
rules are never used in decoding. % TODO give an example
...
distribution % TODO add the word conditional in background
...
codec~\citep{Deutsch:1996:ZCD:RFC1950} for all our % TODO check that the gzip cite is correct
experiments.
%Block compression is also used in other related
%software~\citep{pauls-klein:2011:HLTACL}. % TODO reread paper and elaborate
...
%\emph{hierarchical reordering rules}. % TODO mention this in background
...
all keys are sorted after the reduce step. % TODO clear as mud, expand
...
% TODO explain better the source pattern instantiation business with an example.

% TODO maybe refer to background for pattern definition
...
\paragraph{Parallel Text} We use a small parallel corpus of 750,950 word-aligned sentence
    ...
    NIST'12 Chinese-English evaluation, in order to investigate how systems perform % TODO footnote or cite for nist
...
\begin{itemize}
  ...
  \item $s_{\text{max NT}} = 10$ % TODO harmonize notation with extraction from posteriors and from background
\end{itemize}
%
...
\paragraph{Grammar filtering} For \emph{ruleXtract}, we use the following constraints for
    ...
    \begin{itemize}
      ...
      \item $s_{\text{max span}} = \infty$ % TODO explain diff between hrmaxheight and max nt: max nt is the max span of an X on the rhs whereas hrmaxheight is the max span of the source of a rule, i.e. max span of X on the lhs
    \end{itemize}
    %
    ...
    are comparable. % TODO explain better.
%
...
cutoffs, such as a minimum source-to-target translation probability, % TODO refer to background, TODO in background, actually talk about these cutoffs
...
set.
% TODO expand a bit and maybe make a table to compare timings
